...
final class UITableDDSViewModelTests: XCTestCase {
	...
	private let mockSection = DummySection(title: "mockSection")
	...
	func test_apply_whenAppendingItemsToMainSection_shouldDiffCorrectly() {
		...
		snapshot.appendSections([mockSection])
	...
	func test_add() {
		...
		sut.add([mockItems[0]], to: mockSection) {
			e1.fulfill()
		}
		wait(for: [e1], timeout: 1)
		...
		sut.add([mockItems[1]], to: mockSection) {
			e3.fulfill()
		}
	...
	func test_remove() {
		...
		sut.remove([mockItems[0]], from: mockSection) {
			e1.fulfill()
		}
		...
		sut.remove([mockItems[2]], from: mockSection) {
			e2.fulfill()
		}
		wait(for: [e2], timeout: 0.001)
		XCTAssertEqual(sut.items.count, 2)
		...
		sut.remove([mockItems[1]], from: mockSection ) {
			e3.fulfill()
		}
		wait(for: [e3], timeout: 0.001)
		XCTAssertEqual(sut.items.count, 1)
	...
	func test_snapshot() {
		...
		snapshot2.appendSections([mockSection])
		...
		XCTAssertEqual(snapshot4.sectionIdentifiers, [mockSection])
		XCTAssertEqual(
		...
		XCTAssertEqual(snapshot5.sectionIdentifiers, [mockSection])
		XCTAssertEqual(
		...
		XCTAssertEqual(snapshot6.sectionIdentifiers, [mockSection])
		XCTAssertEqual(
		...
		XCTAssertEqual(snapshot7.sectionIdentifiers, [mockSection])
		XCTAssertEqual(
